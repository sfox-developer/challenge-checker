<x-app-layout>
    ...
    <div class="py-12">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8 space-y-6">
                ...
                <div class="p-6">
                    <div class="flex items-start justify-between mb-6">
                        <div class="flex items-center space-x-4">
                            <img src="{{ $user->getAvatarUrl() }}" alt="{{ $user->name }}" class="h-24 w-24 rounded-full ring-4 ring-white shadow-lg">
                            <div>
                        ...
                        @if(auth()->id() === $user->id)
                            <a href="{{ route('profile.edit') }}" class="px-6 py-2 bg-blue-600 hover:bg-blue-700 text-white font-semibold rounded-lg transition-colors duration-200 flex items-center space-x-2">
                            ...
                            <div>
                                @if($isFollowing)
                                    <form action="{{ route('social.unfollow', $user) }}" method="POST">
                                        @csrf
                                        <button type="submit" class="px-6 py-2 border border-gray-300 text-gray-700 font-semibold rounded-lg hover:bg-gray-50 transition-colors duration-200">
                                            Following
                                        </button>
                                    </form>
                                @else
            ...
            <!-- Tabs -->
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg" x-data="{ activeTab: 'challenges' }">
                <!-- Tab Navigation -->
                ...
                <div class="p-6">
                    ...
                    <div x-show="activeTab === 'challenges'" class="space-y-4">
                        @forelse($publicChallenges as $challenge)
                            <div class="bg-gray-50 overflow-hidden rounded-lg hover:bg-gray-100 transition-colors duration-200">
                                <div class="p-6">
                                    <div class="flex items-start justify-between">
                                        <div class="flex-1">
                                            <h4 class="text-lg font-semibold text-gray-900 mb-2">{{ $challenge->name }}</h4>
                                            @if($challenge->description)
                                                <p class="text-gray-600 mb-3">{{ Str::limit($challenge->description, 150) }}</p>
                                            @endif
                                        ...
                                        <a href="{{ route('challenges.show', $challenge) }}" class="ml-4 text-blue-600 hover:text-blue-500 font-medium">
                                            View →
                                        </a>
                                    </div>
                                </div>
