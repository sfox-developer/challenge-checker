...
@if($goals->isNotEmpty())
    <div class="space-y-4" data-challenge-id="{{ $challenge->id }}">
        <!-- Progress Header -->
        <div class="rounded-lg p-4 border-2 transition-all duration-200 {{ $allCompleted ? 'bg-teal-50 dark:bg-teal-900/20 border-teal-500' : 'bg-gradient-to-r from-blue-50 to-purple-50 dark:from-blue-900/20 dark:to-purple-900/20 border-blue-200 dark:border-blue-800' }}">
            <div class="flex items-center justify-between mb-2">
                <div class="flex items-center gap-2">
                    <svg class="w-5 h-5 {{ $allCompleted ? 'text-teal-500' : 'text-blue-500' }}" fill="currentColor" viewBox="0 0 20 20">
                        <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd"/>
                    </svg>
                    <span class="font-semibold text-gray-900 dark:text-white">Today's Progress</span>
                </div>
                <span class="text-xs font-semibold {{ $allCompleted ? 'text-teal-600 dark:text-teal-400' : 'text-blue-600 dark:text-blue-400' }}" data-progress-text>
            ...
            <div class="bg-gray-200 dark:bg-gray-700 rounded-full h-1.5 overflow-hidden">
                ...
                <div class="mt-2 flex items-center gap-2 text-sm font-medium text-teal-600 dark:text-teal-400">
        ...
        <div class="space-y-2">
            @foreach($goals as $goal)
                ...
                <div class="p-4 rounded-lg border-2 transition-all duration-200 
                    {{ $isCompleted ? 'bg-teal-50 dark:bg-teal-900/20 border-teal-500' : 'bg-white dark:bg-gray-700 border-gray-200 dark:border-gray-600 hover:border-blue-300 dark:hover:border-blue-700' }}"
                    ...
                    <div class="flex items-start gap-3">
                        ...
                        <button type="button" 
                            ...
                            <div class="w-6 h-6 rounded-md border-2 transition-all duration-200 flex items-center justify-center
                                {{ $isCompleted ? 'bg-teal-500 border-teal-500' : 'border-gray-300 dark:border-gray-500 hover:border-blue-500' }}">
                                @if($isCompleted)
                        ...
                        <!-- Completed Badge -->
                        @if($isCompleted)
                            <span class="flex-shrink-0 text-xs font-semibold text-teal-600 dark:text-teal-400">
                                ✓ Done
                            </span>
                        @endif
                    </div>
