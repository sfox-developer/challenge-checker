            ...
            <div class="flex justify-between items-center mt-6 pt-4 border-t border-gray-200 dark:border-gray-700">
                @if(!$challenge->started_at)
                    ...
                    <form method="POST" action="{{ route('challenges.start', $challenge) }}">
                        @csrf
                        <x-ui.app-button variant="action-sm" type="submit">
                            ▶️ Start
                        </x-ui.app-button>
                    </form>
                @elseif($challenge->is_active)
                    ...
                    <div class="flex space-x-2">
                        <form method="POST" action="{{ route('challenges.pause', $challenge) }}">
                            @csrf
                            <x-ui.app-button variant="action-pause" type="submit">
                                ⏸️ Pause
                            </x-ui.app-button>
                        </form>
                        <x-ui.app-button variant="action-complete" type="button" onclick="showCompleteModal()">
                            ✅ Complete
                        </x-ui.app-button>
                    </div>
                @else
